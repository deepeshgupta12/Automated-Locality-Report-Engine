...
import { marketSupply, rentalStats } from "@/lib/reportData";
...
const MarketSnapshot = () => {
  ...
  const parseRent = (str: string) => {
    const cleaned = str.replace("₹ ", "").replace(" K", "").replace(" L", "");
    if (str.includes("L")) return parseFloat(cleaned) * 100000;
    if (str.includes("K")) return parseFloat(cleaned) * 1000;
    return parseFloat(cleaned);
  };

  const rentData = rentalStats.map((r: any) => ({
    type: r.unitType,
        ...
        <div className="rounded-xl bg-card p-6 sy-card-shadow space-y-3">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={rentData} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(220 13% 91%)" />
              <XAxis type="number" tick={{ fontSize: 11, fill: "hsl(220 10% 46%)" }} tickFormatter={(v) => `₹${(v/1000).toFixed(0)}K`} />
              <YAxis dataKey="type" type="category" tick={{ fontSize: 11, fill: "hsl(220 10% 46%)" }} width={60} />
